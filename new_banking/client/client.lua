...
RegisterCommand('atm', function()
	if bankMenu then
		if nearBank() or nearATM() then
			...
			exports['mythic_notify']:SendAlert('inform', _U('bank_too_far'), 2500, { ['background-color'] = '#ce0808', ['color'] = '#f2f2f2' })
		end
...
--ATM
RegisterCommand('atmblip', function()
    if isBlipOn == false then
        for k, v in pairs(Config.ATM) do
            ...
            EndTextCommandSetBlipName(blip)
            table.insert(BlipInfo, {_blip = blip})
        end
        isBlipOn = true
    elseif isBlipOn then
